      ...
      <b-card no-body class="overflow-hidden" style="max-width: 800px">
        <b-row no-gutters>
          ...
          <b-col md="6">
            <b-card-body>
              <b-card-text>
                <h1>{{ getLoggedInUser.username }}</h1>
                <p class="title">
                  {{ userDescription }} ( {{ getLoggedInUser.gender }} )
                </p>
                <p>Email : {{ getLoggedInUser.email }}</p>
                <p>Contact : {{ getLoggedInUser.number }}</p>
                <p>
                  Location : {{ getLoggedInUser.state }},
                  {{ getLoggedInUser.country }}
                </p>
                <p>
